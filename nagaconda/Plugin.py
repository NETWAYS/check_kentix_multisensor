...
class Plugin:
    ...
    def __init__(self, description, version, helptext=''):
        """
        ...
        self.__helptext = helptext # Helptext
    ...
    def __check_range(self, range_type, name):
        ...
        # bottom, top, inside, so long as the array element is defined.
        # Perform our range test and set the exit status.

        (bottom, top, inside) = range_list[threshold-1]

        if inside and val >= bottom and val <= top:
            if self.__exit_status != 'critical':
                self.__exit_status = range_type.split('_', 1)[0]
            self.__perf[name]['state'] = range_type.split('_', 1)[0]
        elif not inside and val < bottom or val > top:
            if self.__exit_status != 'critical':
                self.__exit_status = range_type.split('_', 1)[0]
            self.__perf[name]['state'] = range_type.split('_', 1)[0]
    ...
    def enable_status(self, status_type, required=False):
        ...
        self.add_option(status_type[0], status_type,
            "Set the %s notification level. Syntax: <option>=<range>" %
            status_type, required=required, action='callback',
    ...
    def finish(self):
        ...
        for (perf_name, perf_dict) in list(self.__perf.items()):

            perfs.append('%s=%s%s;%s;%s;%s;%s' % (
                perf_name, perf_dict['val'], perf_dict['scale'] or '',
                self.options.ensure_value('raw_warning_' + perf_name, ''),
                self.options.ensure_value('raw_critical_' + perf_name, ''),
    ...
    def set_value(self, name, val, **kwargs):
        ...
        if hasattr(self.options, 'warning_' + name):
            self.__check_range('warning_' + name, name)

        if hasattr(self.options, 'critical_' + name):
            self.__check_range('critical_' + name, name)
    ...
    def start(self):
        ...
        """
        self.__opt_parser.set_usage(self.__opt_parser.get_usage() +
            self.__helptext)
...
def convert_range(option, opt_str, value, parser):
    ...
    parser.values.ensure_value("raw_%s_%s" % (option.dest, name), value)

    for part in value.split(','):
        # If we find a '@' at the beginning of the range, it's inside
        ...
        parser.values.ensure_value(option.dest + '_' + name, []).append([bottom,
            top, inside])
